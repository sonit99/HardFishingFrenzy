...
export const FishStats = {
    [FishName.Atlantic_Mackerel]: { id: 1, length: 30, value: 25, rarity: FishRarity.Common, call: FishName.Atlantic_Mackerel },
    [FishName.Neon_Tetra]: { id: 2, length: 6.5, value: 15, rarity: FishRarity.Common, call: FishName.Neon_Tetra },
    [FishName.Swordfish]: { id: 3, length: 90, value: 50, rarity: FishRarity.Uncommon, call: FishName.Swordfish },
    [FishName.Leafy_Sea_Dragon]: { id: 4, length: 35, value: 30, rarity: FishRarity.Uncommon, call: FishName.Leafy_Sea_Dragon },
    ...
    [FishName.Ocean_Seraph]: { id: 12, length: 38, value: 400, rarity: FishRarity.Legend, call: FishName.Ocean_Seraph },
    [FishName.Voidfin_Levi]: { id: 13, length: 1400, value: 1000, rarity: FishRarity.Legend, call: FishName.Voidfin_Levi },
};
...
export const MapRate = {
    [Area.Near]: { [FishRarity.Common]: 0, [FishRarity.Uncommon]: 0, [FishRarity.Rare]: 30, [FishRarity.Epic]: 65, [FishRarity.Legend]: 5 },
    [Area.MEDIUM]: { [FishRarity.Common]: 0, [FishRarity.Uncommon]: 0, [FishRarity.Rare]: 30, [FishRarity.Epic]: 65, [FishRarity.Legend]: 5 },
    [Area.FAR]: { [FishRarity.Common]: 0, [FishRarity.Uncommon]: 0, [FishRarity.Rare]: 30, [FishRarity.Epic]: 65, [FishRarity.Legend]: 5 },
    [Area.FARTHEST]: { [FishRarity.Common]: 0, [FishRarity.Uncommon]: 0, [FishRarity.Rare]: 30, [FishRarity.Epic]: 65, [FishRarity.Legend]: 5 },
};
...
export default class FishController extends cc.Component {
    ...
    spawnInitialFish() {
        ...
        for (const area of Object.values(Area).filter(v => typeof v === "number")) {
    ...
    onHookInWater(hook: cc.Node) {
        ...
        let fishWorldPos = nearest.node.parent.convertToWorldSpaceAR(nearest.node.position);
        ...
        if (nearest.node.x < tipPos.x && nearest.node.scaleX > 0 ||
    ...
    update(dt: number) {
        ...
        for (const { node: fish } of this.fishes) {
            const data = this.fishSwimData.get(fish);
            if (!data) continue;

            // === Bơi ngang ===
            fish.x += data.dir * this.swimSpeed * dt;

            // === Dao động nhẹ lên xuống (lượn sóng) ===
            const t = (Date.now() / 1000) + fish.uuid.length * 0.1;
            const offsetY = Math.sin(t * 2) * this.swimAmplitude * 0.5;
            fish.y = data.baseY + offsetY;

            // === Đảo hướng nếu chạm biên ===
            if (fish.x > halfX - padding) {
                data.dir = -1;
            } else if (fish.x < -halfX + padding) {
                data.dir = 1;
            }

            // === Quay đầu mượt theo hướng bơi ===
            const targetScale = data.dir > 0 ? -1 : 1;
            const currentScale = fish.scaleX;
            // lerp scaleX để không giật
            fish.scaleX = cc.misc.lerp(currentScale, targetScale, 0.2);

            // === Nghiêng nhẹ theo hướng di chuyển (cho tự nhiên)
            const tiltAngle = Math.sin(t * 3) * 5 * data.dir; // góc lượn nhẹ
            fish.angle = cc.misc.lerp(fish.angle, tiltAngle, 0.3);

            this.fishSwimData.set(fish, data);
        }
    }
    ...
    getFishNamesByRarity(rarity: FishRarity): string[] {
        return Object.keys(FishStats).filter(name => FishStats[name].rarity === rarity);
    }
    ...
    spawnFishByArea(area: Area, count: number) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            const y = (Math.random() * (this.spawnRangeY - padding * 2)) - (halfY - padding);
